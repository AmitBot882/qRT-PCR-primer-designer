...
with st.expander("⚙️ Lab Conditions & Advanced Thermodynamics (Applied to all)", expanded=True):
    ...
    with col1:
        species_name = st.text_input(
            "Global Species or taxid (for BLAST):", "txid4113",
            help="Scientific name or taxid. taxid is safer — a misspelled name "
...
if blast_mode != "Off" and not species_name.strip():
    st.warning(
        "No organism set — BLAST will search all of `nt`. Every primer will look "
        "non-specific. Enter a taxid or scientific name."
    )
...
if st.button("🚀 Run SPUD Engine", type="primary") and genes_data:
    ...
    for idx, gene in enumerate(genes_data):
        ...
        if len(clean_seq) < 50:
            all_results.append(blank_row(gene['Gene_ID'], "Error: Sequence too short"))
        ...
        if target_type == "Junction":
            if target_val < 20 or target_val > len(clean_seq) - 20:
                all_results.append(blank_row(gene['Gene_ID'], "Error: Junction too close to edge"))
                progress_bar.progress((idx + 1) / len(genes_data))
                continue
            ...
            if roi_len > 0 and roi_start + roi_len <= len(clean_seq):
                ...
                all_results.append(blank_row(gene['Gene_ID'], "Error: Invalid ROI range"))
                progress_bar.progress((idx + 1) / len(genes_data))
                continue

        elif target_type == "Error":
            all_results.append(blank_row(gene['Gene_ID'], "Error: Invalid Target format"))
            progress_bar.progress((idx + 1) / len(genes_data))
            continue
        ...
        try:
            ...
            for i in range(raw_res.get('PRIMER_PAIR_NUM_RETURNED', 0)):
                ...
                row = blank_row(gene['Gene_ID'], "Pending")
                row.update({
            ...
            all_results.append(blank_row(gene['Gene_ID'], f"Error: {e}"))
    ...
    for row in all_results:
        ...
        if should_blast:
            status_text.text(f"BLASTing {row['Gene_ID']} (Rank {row['Rank']})...")
            label, flag = blast_primer_pair(row['Forward_Seq'], row['Reverse_Seq'],
                                            species_name, blast_cache)
            row['Status'] = label
            row['BLAST_Flag'] = flag
        else:
...
if 'spud_results' in st.session_state:
    ...
    if 'BLAST_Flag' in df_results.columns:
        n_unver = (df_results['BLAST_Flag'] == "UNVERIFIED").sum()
        if n_unver:
            st.warning(
                f"{n_unver} pair(s) returned **0 BLAST hits**. That is an "
                f"unverified result, not a clean one — check that the organism "
                f"field (`{species_name}`) matches an NCBI taxonomy entry."
            )
